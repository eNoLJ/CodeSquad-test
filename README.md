...
- 요구되는 모든 기능을 구현했다.
- 입력받는 값에 대해 여러가지 경우로 테스트를 진행하여 오류를 찾기 힘들다.

### 아쉬운 점

- 커밋 메세지 쓰는 타이밍, 형식이 일관되지 못하였다.
- 루빅스 큐브 회전 큐칙을 좀 더 간소화 할 수 있을 것 같은데 못한 아쉬움이 있다.
- 전역 변수를 최대한 쓰지 말라는 요구사항이 있었지만 큐브 틀에 대해 윈도우 객체를 사용했다는 아쉬움이 있다. (웹상의 로컬스토리지를 사용할까하다 말았다...)
